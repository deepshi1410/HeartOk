...
function fetch(){
    ...
    if(username.trim() == ""){
        toastr.warning("Enter Addhar Number");
        return;
    }
    console.log(username);
    
    var q = new Parse.Query("Data");
  q.equalTo("UserId", username);
...
function popup(){
    ...
    if (document.getElementById("save").files.length > 0) {
        ...
        var active = new user();
        active.set("UserId",document.getElementById("uploadBy").value);
        active.set("Records",parseFile);
        active.set("UploadBy",Parse.User.current().get("username"));
